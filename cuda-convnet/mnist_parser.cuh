#ifndef MNIST_PARSER_CUH_
#define MNIST_PARSER_CUH_
...
void _load(std::string fimage, std::string flabel, std::vector<std::vector<float> > & data, std::vector<float>& labels){
	std::fstream in;
	...
	while (!in.eof()){
		...
		index++;
		sample.push_back((float_t)pixel);
		if (index % (28 * 28) == 0){
